...
export default function GestionCuentaArtesano() {
  ...
  useEffect(() => {
    const loadAttempts = async () => {
      if (!userId) return;
      const attempts = await AsyncStorage.getItem(`deleteFailedAttempts_${userId}`);
      if (attempts) {
      ...
      interval = setInterval(() => {
        setDeleteBlockTimeRemaining(prev => {
          if (prev <= 1) {
            ...
            if (userId) AsyncStorage.removeItem(`deleteFailedAttempts_${userId}`);
            return 0;
          }
          return prev - 1;
        });
  ...
  const handleOpenDeleteModal = () => {
    if (deletePasswordBlocked) {
      Alert.alert('Acceso bloqueado', `Has excedido el número de intentos. Inténtalo de nuevo en ${Math.ceil(deleteBlockTimeRemaining / 60)} minutos.`);
      return;
    }
  ...
  const handleEditProfile = async () => {
    ...
    try {
      ...
      Alert.alert("Error", error.message || "No se pudo cargar el perfil para editar.");
    } finally {
  ...
  const handleVerifyDeletePassword = async () => {
    ...
    try {
      ...
      if (error || !data) {
        ...
        if (userId) await AsyncStorage.setItem(`deleteFailedAttempts_${userId}`, currentAttempts.toString());

        if (currentAttempts >= 3) {
          setDeletePasswordBlocked(true);
          setDeleteBlockTimeRemaining(300); // 5 minutos
          setShowDeleteProfile(false);
          Alert.alert('Acceso Bloqueado', 'Has excedido el número de intentos. El acceso a esta función se ha bloqueado por 5 minutos.');
        } else {
          Alert.alert('Contraseña Incorrecta', `Te quedan ${3 - currentAttempts} intentos.`);
        }
        setDeletePasswordValidated(false);
      } else {
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            setShowDeleteProfile(false);
            try {
              const { error } = await artesanoService.deleteArtesanoProfile(session.user.id);
              if (error) throw new Error(error);
              ...
              Alert.alert('Error', e.message || 'No se pudo eliminar el perfil.');
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        >
          ...
          <MaterialCommunityIcons name="account-edit-outline" size={24} color={"#9D046D"} />
          )}
