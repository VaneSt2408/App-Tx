...
import { View, Text, StyleSheet, ScrollView, Linking, TouchableOpacity, SafeAreaView, LayoutAnimation, Platform, UIManager } from 'react-native';
  ...
  return (
    <View style={styles.faqContainer}>
      <TouchableOpacity style={styles.faqItem} onPress={onPress} activeOpacity={0.8}>
        ...
        <Ionicons name={isExpanded ? "chevron-up-outline" : "chevron-down-outline"} size={20} color="#9D046D" />
      </TouchableOpacity>
      {isExpanded && (
  ...
  return (
    <SafeAreaView style={getStyles.safeArea}>
      ...
      <View style={getStyles.header}>
        <TouchableOpacity onPress={() => router.back()} style={getStyles.backButton}>
      ...
      <ScrollView style={getStyles.container} contentContainerStyle={getStyles.contentContainer}>
        ...
        {Object.keys(faqData).filter(key => key !== "Gestión de la Cuenta").map(sectionTitle => (
          <View style={getStyles.section} key={sectionTitle}>
            ...
            {faqData[sectionTitle].map((item, index) => (
              <FaqItem
                ...
                question={item.q}
                answer={item.a}
                isExpanded={expandedIndices.includes(`${sectionTitle}-${index}`)}
        ...
        <View style={getStyles.section}>
          ...
          <View style={getStyles.staticFaqItem}>
            <Text style={getStyles.question}>¿Qué hago si encuentro un error en la app?</Text>
            <Text style={getStyles.answer}>
              Agradecemos que nos informes. Por favor, contáctanos a través del correo de soporte que se encuentra más abajo, detallando el problema que encontraste y, si es posible, adjuntando una captura de pantalla.
            </Text>
          </View>

          <Text style={getStyles.paragraph}>
            Si no encuentras la respuesta a tu pregunta, nuestro equipo está aquí para ayudarte.
          </Text>
